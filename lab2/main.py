...
async def read_items():
    conn = sqlite3.connect('lab2/items.db')
    c = conn.cursor()
...
async def read_cart_items():
    ...
    for item in cart:
        ...
        cart_items.append({"item": {"name": item_info[1], "price": item_info[2]}, "quantity": item[2],
                           "total": item_info[2] * item[2]})
    conn.close()
    return cart_items


@app.get("/cart_items_sorted/", tags=["sorting"])
async def read_cart_items_sorted(page: int = 1, limit: int = 10, sort_by: str = "name", order: str = "asc"):
    conn = sqlite3.connect('lab2/items.db')
    c = conn.cursor()
    c.execute("SELECT * FROM cart")
    cart = c.fetchall()
